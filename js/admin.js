(function () {
  ...
  function safeGetAll() {
    try { return (window.StorageAPI && window.StorageAPI.getAll) ? window.StorageAPI.getAll() : []; }
    catch (e) { return []; }
  }
  ...
  function render() {
    ...
    for (var i = 0; i < mods.length; i++) {
      (function (m) {
        ...
        var tagsHtml = '';
        var t = m.tags || [];
        for (var k = 0; k < t.length; k++) tagsHtml += '<span class="tag">' + t[k] + '</span>';
        ...
        row.querySelector('.dump').onclick = function () {
          ...
          a.href = url; a.download = (m.id || 'module') + '.json'; a.click();
